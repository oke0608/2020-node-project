...
//      이미 등록된 id가 있는 경우 409(Conflict)
const signup = (req, res) => {
  const { name, id, password } = req.body;
  if (!name || !id || !password) return res.status(400).send("필수값이 입력되지 않았습니다");
  Usermodel.findOne({ id }, (err, result) => {
    if (err) return res.status(500).send("사용자 조회시 오류가 발생했습니다");
    if (result) return res.status(409).send("이미 사용중인 ID입니다");
    //등록(비밀번호 암호화)
    const saltRounds = 10;
    bcrypt.hash(password, saltRounds, (err, hash) => {
      if (err) return res.status(500).send("암호화 시 오류가 발생했습니다.");
      const user = new Usermodel({ name: name, id: id, password: hash })
...
const login = (req, res) => {
  ...
  if (!id || !password) return res.status(400).send("필수값이 입력되지 않았습니다.");
